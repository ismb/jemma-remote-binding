...
public class JemmaEnquirer 
{
	...
	{
		this.jemmaHttpContext = new HttpClientContext();
	...
	public List<Appliance> getDevices() throws MethodNotSupportedException, IOException
	{
		...
		try 
		{
			JSONArray jsonAppliancesList = response.getResponseAsJSONObject().getJSONObject(RESULT_KEY).getJSONArray("list");
	...
	private void processJemmaResponse(JemmaMethod calleeMethod, JemmaResponse response)
	{
		try
		{
			...
			case OSGI_FIND:
			{
				...
				JSONObject map = response.getResponseAsJSONObject().getJSONObject("result").getJSONArray("list").getJSONObject(0).getJSONObject("map");
	...
	private HttpEntity forgeHttpEntity(JemmaMethod jemmaMethod, List<Object> params)
	{
		...
		try
		{
			...
			jsonEntity.put(METHOD_KEY, serviceIdString+(serviceIdString.length()>0?".":"")+jemmaMethod.methodName);
			for(Object o: params)
				jsonParams.put(o);
	...
	private void login() throws  InvalidCredentialsException, IOException
	{
		...
		try 
		{
			...
			loginURI = new URI("http://"+jemmaURI.getHost()+":"+jemmaURI.getPort()+
					"/demo/conf/login.html?"+
					USERNAME_KEY+"="+jemmaAuthenticator.username+"&"+
					PASSWORD_KEY+"="+jemmaAuthenticator.password+"&submit=Entra");
